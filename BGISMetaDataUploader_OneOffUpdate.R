...
all_systems <- all_systems_raw %>% filter(is.na(entouch_unit_id))
...
fan_systems <- all_systems %>% filter(class == "fan_systems")
#submeter_systems <- conf_systems %>% filter(class == "submeters")
submeter_systems <- all_systems %>% filter(class == "submeters")
...
meter_cat <- merge(submeter_systems_slim, meters_df[,c(3:9)], by = c("entouch_facility_id", "entouch_unit_id", "entouch_branch"))
    colnames(meter_cat)[c(6,9,14)] <- c("old_name","old_meter_type", "new_meter_type")
...
# TODO: no systems use this yet, build out once there is a need
...
result <- BMUpload(fans, all_names, submeter_cat_final[,c(1:2)], submeter_schedules_final, submeter_schedules_final_9)
...
reqstring <- "http://production-building-model-service.fo.enernoc.net:8080/building-model-service/api/v3/point-conversions/by-customer-id?id=188867437"
mycall<-GET(reqstring)
full_response<-fromJSON(content(mycall, "text"), flatten=TRUE)

json_response<-full_response[[1]]
systems <- json_response %>% filter(!is.na(source_id))
submeters <- systems %>% filter(point_code == "Demand_kW_AI")
submeters2 <- unnest(submeters)
write.csv(submeters2, "C:/Users/anthony.covarrubias/EnerNOC, Inc/BOP - Documents/Projects/TD Bank/Building Model/Enablement/Parameter Updates/20190522-td_submeter_sourceids_demand.csv")

# 
# write.csv(submeter_systems, "C:/Users/anthony.covarrubias/EnerNOC, Inc/BOP - Documents/Projects/TD Bank/Building Model/Enablement/Parameter Updates/20190522-td_submeter_systems.csv")
